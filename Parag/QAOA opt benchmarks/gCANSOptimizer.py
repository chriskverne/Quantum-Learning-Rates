...
class gCANSOptimizer:
    ...
    def step(self, qnode, theta):
        ...
        for i in range(d):
            ...
            s_next_i = np.ceil(factor * (xi[i] * np.sum(xi)) / chi_norm_sq)
            
            # Bound below by s_min to maintain valid sampling size
            s_next[i] = max(self.s_min, s_next_i)
