...
# Dependencies:
# pip install pennylane pennylane-lightning-gpu matplotlib numpy

# ==============================================================================
# 1. CONFIGURATION & SCALING SETUP
# Team Lead Directive: Run with (4q, 2l), (6q, 3l), (8q, 4l), (10q, 5l), (12q, 6l)
# Set your desired benchmark run here:
# ==============================================================================
num_qubits = 4
num_layers = 2
n_steps =200

# Device selection: lightning.gpu for speed on 8q, 10q, 12q
dev = qml.device("lightning.gpu", wires=num_qubits)

# Define Problem Topology: Scalable Ring / Cycle Graph (0-1, 1-2, ..., (N-1)-0)
graph = [(i, (i + 1) % num_qubits) for i in range(num_qubits)]
...
# Compute ground state energy and spectral gap for Polyak step switching
...
for run in range(5):
    print(f"\n--- Starting Independent Run {run + 1}/5 ---")
    ...
    for step in range(n_steps):
        ...
        costs.append(cost)
        if step % 10 == 0:
            print(f"[Polyak-GD] Step {step:03d}: Cost = {cost:.6f}")
...
print(f"\nAverage Best cost = {best_curve[-1]:.6f} (GSE = {eigvals[0]:.6f})")
...
plt.legend(fontsize=12, framealpha=1, loc="upper left", bbox_to_anchor=(1.02, 1), borderaxespad=0.0)
